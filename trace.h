...
#define MMXVAL(env,regid) \
  (((FPReg *)(&(env->fpregs)))[(regid) - 164].mmx.q)
#define XMMVAL(env,regid,idx) \
  (((XMMReg *)(&(env->xmm_regs)))[(regid) - 172]._q[(idx)])
#define FLOATVAL(env,regid) \
  (((FPReg *)(&(env->fpregs)))[(regid) - 188].d)
...
// Write trailer to trace and close it
int close_trace(FILE *stream, ProcRecord ** prarr, size_t num_procs);
